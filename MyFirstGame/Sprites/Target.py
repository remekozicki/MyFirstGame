...
class Target(pygame.sprite.Sprite):
    ...
    def set_target_attributes(self, type):
        if type == 0:
            self.image = pygame.image.load(ImagesPaths().enemies[0]).convert_alpha()
            self.money_per_kill = 10
            self.hp = 50
        elif type == 1:
            self.image = pygame.image.load(ImagesPaths().enemies[1]).convert_alpha()
            self.money_per_kill = 15
            self.hp = 100
        elif type == 2:
            self.image = pygame.image.load(ImagesPaths().enemies[2]).convert_alpha()
